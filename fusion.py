...
class Fusion(object):
    ...
    def __init__(self):
        ...
        self.ts = 0     # Time between updates #NOTE: I MODIFIED THIS
    ...
    def update_nomag(self, accel, gyro, ts=[]):    # 3-tuples (x, y, z) for accel, gyro
    ...
    def update(self, accel, gyro, mag, ts=None):     # 3-tuples (x, y, z) for accel, gyro and mag data
        ...
        # Integrate to yield quaternion
        deltat = ts- self.ts
        self.ts = ts #update time to current time
        q1 += qDot1 * deltat
        q2 += qDot2 * deltat
        q3 += qDot3 * deltat
        q4 += qDot4 * deltat
